 ...
 */
class bloom_filter_expression_converter : public pruning_expression_builder {
 public:
   ...
   */
  [[nodiscard]] maybe_pruning_expr get_bloom_filter_expr() const { return _bloom_filter_expr; }
 ...
 private:
  ...
  maybe_pruning_expr _bloom_filter_expr;
};
  ...
  cuda::stream_ref stream) const
{
  ...
  bloom_filter_expression_converter bloom_filter_expr{filter.get(), output_dtypes, {literals}};

  // Nothing in the filter can be answered by bloom filters, so all row groups are required
  auto const converted_expr = bloom_filter_expr.get_bloom_filter_expr();
  ...
  return collect_filtered_row_group_indices(
    bloom_filter_membership_table, converted_expr.value(), input_row_group_indices, stream);
}
