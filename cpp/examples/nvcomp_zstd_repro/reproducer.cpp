  ...
  void run(int iterations, bool use_dedicated_stream)
  {
    ...
    for (int iter = 1; iter <= iterations && !stop_flag; iter++) {
      ...
      if (iter % 10 == 0 || iter == 1 || iter == iterations) {
        ...
        auto elapsed =
          std::chrono::duration_cast<std::chrono::seconds>(iter_end - table_start).count();
        std::cout << "[T" << table_id_ << "] iter " << iter << "/" << iterations << " | "
                  << files_this_iter << " files"
                  << " | " << iter_rows << " rows"
                  << " | " << std::fixed << std::setprecision(1) << (iter_bytes / 1024.0 / 1024.0)
                  << " MB"
                  << " | " << iter_ms << " ms"
                          ...
                          rmm::device_async_resource_ref mr)
{
  ...
  for (size_t i = 0; i < table_files.size(); i++) {
    scanners.push_back(
      std::make_unique<TableScanSimulator>(static_cast<int>(i), table_files[i], mr));
  }
...
int main(int argc, char* argv[])
{
  if (argc < 2) {
    ...
    std::cerr << "  CUDA_VISIBLE_DEVICES=2 " << argv[0]
              << " /path/to/files --iterations 50 --threads 5\n";
    return 1;
  }
  ...
  std::cout << "Total data read: " << std::fixed << std::setprecision(2) << total_mb << " MB ("
            << (total_mb / 1024.0) << " GB)\n";
